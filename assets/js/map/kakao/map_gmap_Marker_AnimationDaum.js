...
function runMarker(marker, Pos, PrevPos, id, period) {
    var diffLat = Pos.getLat() - PrevPos.getLat();
    var diffLng = Pos.getLng() - PrevPos.getLng();
...
function moveMarker(marker, diffLat, diffLatPerPeriod, diffLngPerPeriod) {
    ...
    var startPosionLat = marker.aniPrevPos.getLat() + diffLatPerPeriod;
    var startPosionLng = marker.aniPrevPos.getLng() + diffLngPerPeriod;
    marker.aniPrevPos = new daum.maps.LatLng(startPosionLat, startPosionLng);
    
    if (diffLat > 0 ){
        if (marker.aniPos.getLat() > marker.aniPrevPos.getLat())
