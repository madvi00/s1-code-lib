...
function addDistancePaintMarker(latLng) {
    ...
    google.maps.event.addListener(marker, 'click', (function (marker) {
        return function () {
            ...
            var emer_location = new google.maps.LatLng(marker.D_carPos.lat(), marker.D_carPos.lng());

            geocoder.geocode({ 'location': emer_location },
                            function (results, status) {

                                var htmls = "<div style='width:330px; height:230px;'> "

                                if (status == google.maps.GeocoderStatus.OK) {

                                    htmls += " <br>현재위치 : " + results[0].formatted_address;
                                } else if (status == google.maps.GeocoderStatus.OVER_QUERY_LIMIT) {
                                    htmls += " <br>현재위치 : *위치정보 조회 실패"
                                }

                                htmls += " <br>누적거리 : " + marker.D_AddedDistanceText;
                                htmls += " <br><img src='https://maps.googleapis.com/maps/api/streetview?size=300x150&location=" + marker.D_carPos.lat() + "," + marker.D_carPos.lng() + "&heading=" + marker.dBering + "&sensor=false'  />";
                                htmls += "</span></div>";
...
function addDistancePaintPolyline(marker) {
    ...
    if (objDistanceMarkers.get(iMarkerIndex) && objDistanceMarkers.get(iMarkerIndex).index == iToolsIndex) {
        ...
        iDistanceTotal += Math.round(google.maps.geometry.spherical.computeLength(polyLine.getPath()));
        ...
        //alert(google.maps.geometry.spherical.computeLength(polyLine.getPath()));
    ...
    delete points, polyLine, linecolor, lineSymbol, prevDistancePaintMarker;
}
...
function addAreaPolygon(marker) {
    ...
    if (objDistanceMarkers.get(iMarkerIndex) && objDistanceMarkers.get(iMarkerIndex).index == iToolsIndex) {
        ...
        var arrPolygon = new google.maps.MVCArray();
...
function GetViewPopup(event) {
    ...
    if (objDistanceMarkers.get(iMarkerIndex) && objDistanceMarkers.get(iMarkerIndex).index == iToolsIndex) {
        ...
        var arrPolyline = new google.maps.MVCArray();
        ...
        var iDistance = Math.round(google.maps.geometry.spherical.computeLength(arrPolyline)) + iDistanceTotal;
        //iDistanceTotal += Math.round(google.maps.geometry.spherical.computeLength(polyLine.getPath()));
        ...
        //{ 'position': 'absolute', 'top': event.pixel.y+240, 'left': event.pixel.x+30 })
        ...
        var labelHtml = "<div id='divDistanceViewPopup' style='position:absolute;top:" + (event.pixel.y + 5) + "px;left:" + (event.pixel.x + 5) + "px;z-index:1010;background-color:white;text-decoration:blink;'>";
        //if (navigator.appVersion.indexOf("MSIE 8.0") != -1) {
